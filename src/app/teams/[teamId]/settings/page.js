...
import { MessageSquare, FileText, Cloud, Github, Bug, LogOut, Video, Calendar, Pencil, UserCircle, Plus, Minus, UserPlus } from 'lucide-react';
...
function SettingsContent() {
    ...
    const fetchTeamDetails = async () => {
      try {
        const { data: team, error } = await supabase
          .from('teams')
          .select('team_name, location, created_at')
          .eq('id', params.teamId)
          .single();
        ...
        if (team) {
          ...
          setTeamLocation(team.location || 'Not specified');
          setEditedLocation(team.location || '');
          setTeamCreatedAt(new Date(team.created_at).toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
          }));
  ...
  const handleCancel = (field) => {
    if (field === 'name') setEditedName(teamName);
    if (field === 'location') setEditedLocation(teamLocation);
    setEditingField(null);
  };

  const getInitials = (fullName, email) => {
    // If we have a full name, get initials from it
    if (fullName) {
  ...
  const handleCopyInviteLink = () => {
    const link = `${window.location.host}/teams/join/${teamInviteCode}`;
    navigator.clipboard.writeText(link)
      .then(() => {
        setCopyButtonText('Copied!');
        setTimeout(() => setCopyButtonText('Copy URL'), 3000);
  ...
  return (
    <div className="flex min-h-screen bg-black text-white">
          ...
          <div className="text-md space-y-2">
            <button
              ...
              className={`w-full text-left px-4 py-2 rounded-lg ${
                activeTab === 'general' ? 'bg-zinc-800 text-white' : 'text-zinc-400 hover:bg-zinc-800'
              }`}
            >
              General
            </button>
            <button
              ...
              className={`w-full text-left px-4 py-2 rounded-lg ${
                activeTab === 'members' ? 'bg-zinc-800 text-white' : 'text-zinc-400 hover:bg-zinc-800'
              }`}
            >
              Members
            </button>
        ...
        <div className="flex-1 p-8 bg-black overflow-y-auto">
          {activeTab === 'general' && (
            <div className="space-y-8">
              <h1 className="text-2xl font-medium text-white">General</h1>
      ...
      {isInviteModalOpen && (
        <div className="px-2 fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-black bg-opacity-40 backdrop-blur-sm p-8 rounded-lg shadow-lg border border-white/20">
            <h2 className="lg:text-xl text-md font-medium mb-4 text-white">Invite members to <b>{teamName}</b></h2>
